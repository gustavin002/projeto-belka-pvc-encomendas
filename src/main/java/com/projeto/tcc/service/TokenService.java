...
public class TokenService {
 // Injeta o valor do segredo do token a partir do arquivo application.properties
    ...
    public String gerarToken(UsuarioDTO user) {
        ...
        if(
            user.getIdUsuario()== 0 ||
            user.getNomeUsuario().equals("") ||
            user.getEmailUsuario().equals("") ||
            user.getRoleUsuario().equals("")
        ) {
    ...
    public UsuarioDTO extrairClaim(String token) {
        ...
        user.setIdUsuario(claims.get("id", Long.class).intValue());
        // Extrai o nome do claim "nome"
        user.setNomeUsuario(claims.get("nome", String.class));
        // Extrai o papel/função do claim "role"
        user.setRoleUsuario(claims.get("role", String.class));
